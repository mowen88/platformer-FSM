...
from sprites import FadeSurf, Collider, CutsceneCollider, BG, Tile, AnimatedTile, DisappearingPlatform, EscalatorPlatform, MovingPlatform, SawBlade
...
class CreateZone:
	def __init__(self, game, zone):
		...
		self.zone.fade_surf = FadeSurf(self.zone, [self.zone.updated_sprites, self.zone.rendered_sprites], (0,0))
	...
	def create(self):
		...
		for obj in tmx_data.get_layer_by_name('cutscenes'):
			...
			if obj.name == '2': CutsceneCollider([self.zone.cutscene_sprites, self.zone.updated_sprites], (obj.x, obj.y, obj.width, obj.height), obj.name)

		for obj in tmx_data.get_layer_by_name('exits'):
			if obj.name == '0': CutsceneCollider([self.zone.exit_sprites, self.zone.updated_sprites], (obj.x, obj.y, obj.width, obj.height), obj.name)
			if obj.name == '1': CutsceneCollider([self.zone.exit_sprites, self.zone.updated_sprites], (obj.x, obj.y, obj.width, obj.height), obj.name)
			if obj.name == '2': CutsceneCollider([self.zone.exit_sprites, self.zone.updated_sprites], (obj.x, obj.y, obj.width, obj.height), obj.name)
		...
		for _, __, img_files in walk(f'../zones/{self.zone.name}/bg_images'):
			for img in img_files:
				if img == '2x6_white.png': BG(self.game, self.zone, [self.zone.updated_sprites, self.zone.rendered_sprites], (0, 0), pygame.image.load(f'../zones/{self.zone.name}/bg_images/{img}').convert_alpha(), (0.1, 0.1), LAYERS['blocks'])
				if img == 'bg1.png': BG(self.game, self.zone, [self.zone.updated_sprites, self.zone.rendered_sprites], (0, 0), pygame.image.load(f'../zones/{self.zone.name}/bg_images/{img}').convert_alpha(), (0.01, 0.01), LAYERS['BG1'])
				if img == 'bg2.png': BG(self.game, self.zone, [self.zone.updated_sprites, self.zone.rendered_sprites], (0, 0), pygame.image.load(f'../zones/{self.zone.name}/bg_images/{img}').convert_alpha(), (0.02, 0.05), LAYERS['BG2'])
				if img == 'bg0.png': BG(self.game, self.zone, [self.zone.updated_sprites, self.zone.rendered_sprites], (0, 0), pygame.image.load(f'../zones/{self.zone.name}/bg_images/{img}').convert_alpha(), (0.1, 0.03), LAYERS['BG0'])
